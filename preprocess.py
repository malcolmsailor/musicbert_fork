...
PREFIX = os.path.join(os.environ["DATASETS_DIR"], "lmd", "musicbert")
...
DATA_ZIP: Optional[zipfile.ZipFile] = None
OUTPUT_FILE = None
...
def encode_file(file_name, midi_dict, zip_path, output_file):
    try_times = 10
    midi_file = None
    ...
    for sp in ["train", "valid", "test"]:
        ...
        if sp == "valid":  # 1%
            file_list_split = file_list[
                98 * total_file_cnt // 100 : 99 * total_file_cnt // 100
            ]
        if sp == "test":  # 1%
