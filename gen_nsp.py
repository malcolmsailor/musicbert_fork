...
timesig = preprocess.time_sig_to_token((4, 4))
...
for split in ["train", "test"]:
    samples_split = set(
        samples[:samples_size] if split == "train" else samples[samples_size:]
    )
    with open(
        input_file_dir + "/" + ("train" if samples_size > 0 else split)
    ) as f_input:
        with open(output_file_dir + "/" + split + ".txt", "w") as f_txt:
            with open(output_file_dir + "/" + split + ".label", "w") as f_label:
                ...
                for file_line in f_input:
                    ...
                    first_length = sum(
                        _ > 0
                        for _ in data_item[: source_pos_resolution * tokens_per_note]
                    )
                    encoding = []
                    for pair_id in range(2):
                        for note_id in range(source_pos_resolution * tokens_per_note):
                            note = [
                                data_item[
                                    (
                                        (pair_id * tokens_per_note + _)
                                        * source_pos_resolution
                                        * tokens_per_note
                                    )
                                    + note_id
                                ]
                                for _ in range(tokens_per_note)
                            ]
                            ...
                            state_rest = 0
                            if (
                                (note_id == 0 and note_state == state_hold)
                                or note_state == state_on
                            ) and pitch_class in chord:
                                pos = (
                                    note_id
                                    if task == "acc" or pair_id == 0
                                    else first_length + note_id
                                ) * pos_scale
                                measure = pos // (
                                    preprocess.POS_RESOLUTION
                                ...
                                velocity = preprocess.velocity_to_token(
                                ...
                                # find next note/rest
                                while (
                                    k < source_pos_resolution * tokens_per_note
                                    and data_item[
                                        (
                                            (pair_id * tokens_per_note + state_idx)
                                            * source_pos_resolution
                                            * tokens_per_note
                                        )
                                        + k
                                    ]
                                    in [state_off, state_hold]
                                ):
                                    k += 1
                                duration = preprocess.duration_to_token(
                                    (k - note_id) * pos_scale
                                )
                                s = (
                                    chord[pitch_class]
                                    if type(chord[pitch_class]) == set
                                    else set(range(12))
                                )
