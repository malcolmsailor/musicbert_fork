...
for SLURM_ID in ${SLURM_IDS[@]}; do
    ...
    set -x
    python "${LABEL_SCRIPT}" \
        --config-file "${MUSIC_DF_FOLDER}/scripts/configs/label_quantize1.yaml" \
        metadata_path="${output_folder}"/metadata_test.txt \
        labels_path=["${output_folder}"/predictions/bar_delta.txt,"${output_folder}"/predictions/dur_token_target.txt,"${output_folder}"/predictions/pos_token_target.txt] \
...
for SLURM_ID in ${SLURM_IDS[@]}; do
    set -x
    python "${QUANTIZE_DATA_FOLDER}"/scripts/apply_quantization.py \
